...
const UserView: React.FC = () => {
  ...
  // components/UserView.tsx ichidagi handleCheckout funksiyasi uchun:
  const handleCheckout = () => {
    ...
    if (window.Telegram?.WebApp) {
      ...
      window.Telegram.WebApp.close(); // Yuborgandan keyin web appni yopish
    } else {
      alert("Faqat Telegram bot ichida ishlaydi!");
    }
  };
        ...
        <div className="bg-white w-full max-w-md rounded-2xl p-6 space-y-4 animate-scale-up">
          ...
          <div className="flex bg-gray-100 p-1 rounded-xl">
            <button
              ...
              className={`flex-1 py-2 text-xs font-bold rounded-lg ${orderType === 'dostavka' ? 'bg-white shadow-sm' : ''}`}
            >
              Dostavka
            </button>
            <button
              onClick={() => setOrderType('band_qilish')}
              className={`flex-1 py-2 text-xs font-bold rounded-lg ${orderType === 'band_qilish' ? 'bg-white shadow-sm' : ''}`}
            >
              Band qilish
            </button>
          </div>
          ...
          <div className="flex space-x-2">
            <input
              className="flex-1 p-3 border rounded-xl uppercase text-sm"
              placeholder="Promokod"
              value={promoCode}
              onChange={e => setPromoCode(e.target.value)}
            />
            <button onClick={handleApplyPromo} className="px-4 bg-gray-900 text-white rounded-xl text-xs font-bold">Apply</button>
          </div>
          ...
          <button
            ...
            className="w-full bg-gray-900 text-white py-4 rounded-xl font-bold flex items-center justify-center space-x-2"
          >
            <Send size={18} />
