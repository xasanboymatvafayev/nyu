...
// Added ShoppingBag and removed unused Filter from lucide-react imports
...
const UserView: React.FC = () => {
  ...
  const handleCheckout = () => {
    ...
    // Local state uchun order yaratish
    const newOrder = {
      ...
      totalPrice: total,
      orderType,
      userTelegram: window.Telegram?.WebApp?.initDataUnsafe?.user?.username || "Noma'lum",
    ...
    if (window.Telegram?.WebApp) {
      ...
      // window.Telegram.WebApp.close(); // Comment qilingan, chunki test qilishda kerak bo'lishi mumkin
    }

    // 2. Local state ga saqlash
    placeOrder(newOrder);
                ...
                <div className="flex items-center justify-between">
                  ...
                  {selectedProduct.quantity > 0 && (
                    <div className="flex items-center space-x-4">
                      <span className="text-xs text-gray-400">Nechta olmoqchisiz?</span>
                      <input
                        type="number"
                        min="1"
          ...
          <div className="bg-white w-full max-w-md h-full flex flex-col animate-slide-left">
                ...
                <div key={item.id} className="flex space-x-4 bg-gray-50 p-3 rounded-xl">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex justify-between items-center mt-2">
                      ...
                      <button onClick={() => removeFromCart(item.id)} className="text-red-500 text-xs hover:text-red-700 transition-colors">O'chirish</button>
                    </div>
                  </div>
                </div>
              ))}
            ...
            <div className="p-6 border-t space-y-4">
              ...
              <button
                ...
                className="w-full bg-amber-600 text-white py-4 rounded-xl font-bold hover:bg-amber-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
                Buyurtma berish
              </button>
            </div>
      ...
      {isCheckoutOpen && (
        <div className="fixed inset-0 bg-black/60 z-[80] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-md rounded-2xl p-6 space-y-4 animate-scale-up">
            ...
            <input
              className="w-full p-3 border rounded-xl focus:ring-2 focus:ring-amber-500 focus:border-transparent outline-none transition-all"
            ...
            <div className="flex space-x-2">
              <input
                className="flex-1 p-3 border rounded-xl focus:ring-2 focus:ring-amber-500 focus:border-transparent outline-none transition-all"
            ...
            <div className="flex bg-gray-100 p-1 rounded-xl">
              <button
                ...
                className={`flex-1 py-2 text-xs font-bold rounded-lg transition-colors ${orderType === 'dostavka' ? 'bg-white shadow-sm text-gray-900' : 'text-gray-500 hover:text-gray-700'}`}
              >
                Dostavka
              </button>
              <button
                onClick={() => setOrderType('band_qilish')}
                className={`flex-1 py-2 text-xs font-bold rounded-lg transition-colors ${orderType === 'band_qilish' ? 'bg-white shadow-sm text-gray-900' : 'text-gray-500 hover:text-gray-700'}`}
              >
                Band qilish
              </button>
            </div>
            ...
            <div className="flex space-x-2">
              <input
                className="flex-1 p-3 border rounded-xl uppercase text-sm focus:ring-2 focus:ring-amber-500 focus:border-transparent outline-none transition-all"
            ...
            <div className="pt-4 border-t space-y-2">
              ...
              {appliedPromo > 0 && (
                <div className="flex justify-between text-sm text-green-600">
                  <span>Chegirma:</span>
                  <span>-{appliedPromo}% ({Math.round(subTotal * appliedPromo / 100).toLocaleString()} uzs)</span>
                </div>
              )}
              <div className="flex justify-between font-bold text-lg pt-2 border-t">
                <span>To'lanadi:</span>
                <span className="text-amber-700">{Math.round(total).toLocaleString()} uzs</span>
              </div>
            </div>

            <button
              onClick={handleCheckout}
              className="w-full bg-gray-900 text-white py-4 rounded-xl font-bold flex items-center justify-center space-x-2 hover:bg-black transition-colors"
            >
              <Send size={18} />
